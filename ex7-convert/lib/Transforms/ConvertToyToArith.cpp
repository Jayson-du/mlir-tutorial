...
struct AddOpPat : OpConversionPattern<AddOp> {
  ...
  LogicalResult matchAndRewrite(AddOp op, AddOpAdaptor adaptor,
...
struct SubOpPat : OpConversionPattern<SubOp> {
  ...
  LogicalResult matchAndRewrite(SubOp op, SubOpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const {
...
struct ConstantOpPat : OpConversionPattern<ConstantOp> {
  ...
  LogicalResult matchAndRewrite(ConstantOp op, ConstantOpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const {
...
struct ReturnOpPat : OpConversionPattern<ReturnOp> {
  ...
  LogicalResult matchAndRewrite(ReturnOp op, ReturnOpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const {
...
struct CallOpPat : OpConversionPattern<CallOp> {
  ...
  LogicalResult matchAndRewrite(CallOp op, CallOpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const {
    SmallVector<Type> resTypes;
    assert(succeeded(
        getTypeConverter()->convertTypes(op->getResultTypes(), resTypes)));
    rewriter.replaceOpWithNewOp<CallOp>(op, resTypes, op.getCallee(),
                                        adaptor.getOperands());
    return success();
  }
};
...
struct ConvertToyToArithPass
  ...
  void runOnOperation() final {
    ...
    //   return llvm::all_of(f.getArgumentTypes(), [](Type t) {return
    //   !isa<ToyIntegerType>(t);});
    // });
    ...
    converter.addConversion(
        [&](ToyIntegerType t) -> std::optional<IntegerType> {
...
toy::createConvertToyToArithPass(ConvertToyToArithOptions options) {
